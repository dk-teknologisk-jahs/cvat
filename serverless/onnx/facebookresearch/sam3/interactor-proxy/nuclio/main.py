...
"""
SAM3 Interactor Proxy

Lightweight proxy that forwards interactor requests to the SAM3 detector function.
This allows registering SAM3 as an interactor in CVAT without loading models twice.
...
def init_context(context):
    ...
    """
    context.logger.info("SAM3 Interactor Proxy initialized")
...
def handler(context, event):
    """Forward interactor requests to the SAM3 detector function with retry logic."""
    try:
        ...
        # Interactor requests use encode mode (point/box prompts)
        # The detector handler will auto-detect based on request content
        # but we can be explicit
        if "mode" not in data:
            data["mode"] = "encode"

        # Forward to detector function with retry logic
        context.logger.info("Forwarding interactor request to detector function")
        ...
        return context.Response(
            body=json.dumps({
                "error": f"Request timed out after {TIMEOUT}s. The detector may be overloaded."
            }),
            headers={},
            content_type="application/json",
            status_code=504,
